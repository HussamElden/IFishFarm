...
cap = cv2.VideoCapture('normalgedid.mp4')
ret, frame = cap.read()
...
video = cv2.VideoWriter('videonormal/first5secs_'+str(dt_string)+'.avi', fourcc, fps, (fwidth, fheight))
     ...
     if frms%2==0:
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                cv2.putText(img, '{}{}'.format("Eng. Tarek", lbl), (x, y - 5), font, 1, (255, 255, 255), 2)
        ...
        # cap.set(1,frms)
        video.write(img)
        if(frms%(round(fps)*num_seconds)==0):
           dt_string = datetime.now().strftime("%H_%M_%S")
           ...
           if(result==1):
               with open('exceltext/'+str(frms)+'_'+str(dt_string)+'.csv', 'w', newline='') as file:
                    ...
                    # writer.writerows(preproccesing.featuresCalc(preproccesing.work_your_magic(mylist)))
               cv2.imwrite("trajecstest"+str(frms)+'_'+str(dt_string)+".png",mask)

               video.release()
               video = cv2.VideoWriter('videotest/'+str(frms)+'_'+str(dt_string)+'.avi', fourcc, fps, (fwidth, fheight))
